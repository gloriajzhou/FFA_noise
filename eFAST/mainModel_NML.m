...
save mainModel_NML_results.mat;
...
s_NML_1 = efast_ttest(Si,rangeSi,Sti,rangeSti,1:nTimePoints,efast_var,1,y_var_label,0.05);
% For rel_noise (variable 2)
s_NML_2 = efast_ttest(Si,rangeSi,Sti,rangeSti,1:nTimePoints,efast_var,2,y_var_label,0.05);
...
for i=1:k
    ...
    meanSti_var1(i) = mean(s_NML_1.rangeSti(i,:,:,1));
    stdSti_var1(i) = std(s_NML_1.rangeSti(i,:,:,1));
    % Mean and STD for rel_noise (variable 2)
    meanSti_var2(i) = mean(s_NML_2.rangeSti(i,:,:,2));
    stdSti_var2(i) = std(s_NML_2.rangeSti(i,:,:,2));
end
